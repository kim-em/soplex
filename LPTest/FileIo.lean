...
/-- The reference Problem corresponding to `LPTest/fixtures/tiny.mps`
    and `LPTest/fixtures/tiny.lp`: minimize `x1 + x2` subject to
    `x1 + x2 = 1`, `x1, x2 ≥ 0`. -/
...
private def tFixtureMps : IO Outcome := do
  let path : System.FilePath := "LPTest" / "fixtures" / "tiny.mps"
  match readMps path with
  | .error e => return .fail s!"readMps failed: {repr e}"
  | .ok p => return equalAfterValidate tinyProblem p

private def tFixtureLp : IO Outcome := do
  let path : System.FilePath := "LPTest" / "fixtures" / "tiny.lp"
  match readLp path with
  | .error e => return .fail s!"readLp failed: {repr e}"
  | .ok p => return equalAfterValidate tinyProblem p
...
private def tMaximizeFixture : IO Outcome := do
  let path : System.FilePath := "LPTest" / "fixtures" / "maximize.lp"
...
/-! ## Verify-corpus round-trip.
  ...
  Each `Problem` below appears in `LPTest/Verify.lean`'s hand-rolled
